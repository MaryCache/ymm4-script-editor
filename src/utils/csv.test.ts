...
test("buildLineCSV は 名前,セリフ 形式（ASCII カンマは全角化）", () => {
  ...
  expect(buildLineCSV({ id: "x", characterId: "c1", text: "今日は,解説するわ" }, project.characters)).toBe(
    "霊夢,今日は，解説するわ",
  );
});
...
test("buildLineCSV は読点「、」を変換しない（データ保持）", () => {
  ...
  expect(buildLineCSV({ id: "x", characterId: "c1", text: "あ、い" }, project.characters)).toBe("霊夢,あ、い");
});

test("buildLineCSV は改行/CR を半角スペースに畳む", () => {
  expect(buildLineCSV({ id: "x", characterId: "c1", text: "あ\nい\r\nう" }, project.characters)).toBe("霊夢,あ い う");
});

test("buildLineCSV はキャラ名の ASCII カンマも全角化する（名前列の列ズレ防止）", () => {
  const charactersWithComma = [{ id: "cx", name: "田中,太郎", color: "#FF6B6B" }];
  expect(buildLineCSV({ id: "x", characterId: "cx", text: "セリフ" }, charactersWithComma)).toBe("田中，太郎,セリフ");
});
